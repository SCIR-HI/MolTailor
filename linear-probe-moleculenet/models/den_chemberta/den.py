...
Date: 2023-07-31 15:08:03
LastEditors: simwit 517992857@qq.com
LastEditTime: 2023-08-12 18:34:49
FilePath: /workspace/01-st/finetune-moleculenet/models/den_chemberta/den.py
Description: 
...
class DenModel(DenPreTrainedModel):
    def __init__(self, config: DenConfig, smi_encoder):
    ...
    def get_input_embeddings(self):
        return self.txt_encoder.embeddings.word_embeddings
    ...
    def forward(self,
        ...
        if not return_dict:
            return (sequence_output_smi, sequence_output_txt_mlm, sequence_output_txt_cl, sequence_output_txt_mtr)
        
        return DenModelOutput(
            last_hidden_state_smi = sequence_output_smi,
            last_hidden_state_txt_mlm = sequence_output_txt_mlm,
            last_hidden_state_txt_cl = sequence_output_txt_cl,
...
class DenForFineTuning(DenPreTrainedModel):
    def __init__(self, config: DenConfig = config, smi_encoder = None):
        ...
        self.smi_key = nn.Linear(self.smi_config.hidden_size, self.all_head_size)
        self.smi_value = nn.Linear(self.smi_config.hidden_size, self.all_head_size)
    ...
    def forward(self,
        ...
        outputs_smi = self.smi_encoder(**smi).last_hidden_state
        ...
        # extended_attention_mask_smi = (~smi['imask'][:, None, None, :]).to(dtype=extended_attention_mask_txt.dtype)
        extended_attention_mask_smi = smi['attention_mask'][:, None, None, :]
        extended_attention_mask = torch.cat([extended_attention_mask_smi, extended_attention_mask_txt], dim=-1)
